import { Undo2, Redo2, ImageIcon, Download, Sun, Moon, Monitor, Check } from 'lucide-react'
...
export function Header() {
  ...
  const { planningYear, scale, rowHeight, showWeekends, showNRD, showUnpaidLeave, maxConcurrentVacations, theme, setTheme } = useSettingsStore()
      ...
      <div className="flex items-center gap-1">
        ...
        <DropdownMenu.Root>
          <DropdownMenu.Trigger asChild>
            <Button variant="ghost" size="icon" className="h-8 w-8" title="Тема оформления">
            ...
            >
              {THEMES.map((t) => (
                ...
                >
                  {t.icon}
                  {t.label}
                  {theme === t.value && <Check className="h-3.5 w-3.5 ml-auto" />}
                </DropdownMenu.Item>
              ))}
            </DropdownMenu.Content>
          </DropdownMenu.Portal>
        ...
        <DropdownMenu.Root>
          <DropdownMenu.Trigger asChild>
            <Button variant="ghost" size="icon" className="h-8 w-8" title="Экспорт диаграммы">
